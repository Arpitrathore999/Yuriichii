...
@app.on_message(filters.private & filters.text & ~filters.command(["start","ai"]))
...
async def group_chat(_, message):
    ...
    mentioned = me.username and re.search(rf"@{re.escape(me.username)}(?:\s|$|[.,!?])", raw, re.I)
    replied = message.reply_to_message and message.reply_to_message.from_user and message.reply_to_message.from_user.id == me.id
    ...
    if replied:
        ...
        prompt = re.sub(rf"@{re.escape(me.username)}(?:\s+|$)", "", raw, count=1, flags=re.I).strip()
